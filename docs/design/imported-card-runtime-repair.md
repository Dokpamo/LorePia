...
- Page navigation, including returning to offset zero, retains the exact review hash, conflict choices, resolved plan and approval identity. A separate explicit fresh review clears them. Changed review hashes still fail closed.
- The isolated room appends the background's DOM after selecting message overlays and sanitizes the combined tree. Background asset references share the same bounds and approved digest resolver as message assets. Background controls stay out of ordinary chat messages and the host DOM; imported scripts, arbitrary URLs and network access remain blocked.
- A component-owned resize observer tracks actual frame width for CBS conditions. Height-only notifications do not rebuild the frame; the observer is disconnected on unmount. Existing cancellation and last-completed-frame retention remain intact.
- Generated provider preset IDs use the SHA-256 of the complete UTF-8 source ID plus a purpose suffix. Unicode, punctuation and long shared prefixes cannot collapse to the same slug. Reapplying a source is stable. Existing presets are not renamed or deleted; an explicit setup save updates only its selected prompt/room references to the new ID.

Regression coverage includes navigation back to page zero and subsequent activation, stale review rejection, background-only controls/media and combined asset limits, width changes and observer cleanup, plus distinct Unicode/punctuation/long source IDs and stable repeated application. Public DTOs, IPC, schema and native capabilities are unchanged.
