...
#[test]
fn valid_png_full_and_partial_get_validate_the_whole_reserved_image() {
    for range in [None, Some("bytes=2-4")] {
        ...
        assert_eq!(backend.last_read_start.get(), Some(0));
        assert_eq!(backend.last_read_length.get(), Some(70));
        assert_eq!(admission.reserved_bytes(), 70);
