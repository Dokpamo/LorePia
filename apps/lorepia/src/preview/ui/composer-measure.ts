...
export function measureComposer(
    ...
    function measure() {
        ...
        // Use the final writing layout, never an intermediate animated width.
        field.classList.add('ui-compose-measuring');
        ...
        const textLimit = Math.max(line + padding, Math.min(10 * line + padding, available - 72));
        const overflows = scrollHeight > textLimit + 1;
        field.style.setProperty(
            '--ui-compose-text-size',
            `${String(Math.ceil(Math.min(scrollHeight, textLimit)))}px`,
        );
        field.style.setProperty('--ui-compose-limit', `${String(Math.ceil(textLimit + 72))}px`);
        field.classList.remove('ui-compose-measuring');
