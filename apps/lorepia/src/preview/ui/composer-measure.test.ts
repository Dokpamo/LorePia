...
describe('inline composer line growth', () => {
    ...
    it('measures compact toolbar geometry without taking space from the ten writing lines', () => {
        const { input, field, grow, report } = setup();
        const actions = document.createElement('div');
        actions.className = 'ui-compose-actions';
        actions.style.bottom = '7.2px';
        Object.defineProperty(actions, 'offsetHeight', { value: 44 });
        field.append(actions);
        const region = document.createElement('div');
        region.style.top = '7.2px';
        input.before(region);
        region.append(input);
        grow(10);
        expect(report).toHaveBeenLastCalledWith({ lines: 10, overflows: false });
        expect(field.style.getPropertyValue('--ui-compose-limit')).toBe('330px');
        grow(11);
        expect(report).toHaveBeenLastCalledWith({ lines: 11, overflows: true });
    });
});
