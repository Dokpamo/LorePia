...
function readSize(): ChatTextSize {
    try {
        return localStorage.getItem('lorepia.chatTextSize') === 'large' ? 'large' : 'normal';
    } catch {
        return 'normal';
    }
}
...
export function setChatTextSize(value: ChatTextSize): void {
    ...
    try {
        localStorage.setItem('lorepia.chatTextSize', value);
    } catch {
        /* Keep the visual preference usable. */
    }
}
export function initDisplay(): void {
    setChatTextSize(readSize());
}
