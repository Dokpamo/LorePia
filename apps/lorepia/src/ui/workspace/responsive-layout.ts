...
export function responsiveLayout(
    ...
    // Keep page geometry in viewport coordinates. Compact density changes the
    // design tokens together without scaling the gesture/editor coordinate space.
    const width = Math.max(1, size.width);
    const scale = 1;
    const density = Math.min(1, Math.max(0.9, width / 393));
