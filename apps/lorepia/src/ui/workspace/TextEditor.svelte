<script lang="ts">
    import { ArrowLeft, ArrowUp, Check, Minimize2, X } from '@lucide/svelte';
    ...
    function change(next: string) {
        value = next;
        if (!request.applyOnDone) request.onchange(next);
    }
    async function done() {
...
{#if saveError}<p class="ui-live-editor-error" role="alert">{$tr('workspace.saveFailed')}</p>{/if}
...
>
    ...
    >
        ...
        >
            ...
            {:else if !search}
                <IconButton label={$tr('uiPreview.editDone')} disabled={composing} onclick={done}
                    ><Check /></IconButton
                >
            {/if}
        </header>
