                        ...
                        <span class="ui-character-content ui-press-visual" aria-hidden="true">
                            {#if item.cards.length > 1}<span class="ui-folder-grid"
                                    >{#each item.cards.slice(0, 4) as id (id)}<span
                                        ...
                                        >{/each}</span
                                >
                            {:else}<span class="ui-mini-card"
                                    >{#if client && card(item.id)?.avatarAssetId}<CharacterImage
                                            {client}
                                            assetId={card(item.id)?.avatarAssetId ?? ''}
                                            name={card(item.id)?.name ?? ''}
                                        />{:else}{card(item.id)?.thumbnail}{/if}</span
                                >{/if}
                        </span>
                    </button>
