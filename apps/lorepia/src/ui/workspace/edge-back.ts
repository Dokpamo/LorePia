...
import { navigationTiming, navigationUnderlay } from './navigation-motion';
...
export function edgeBack(node: HTMLElement, initial: BackOptions) {
    ...
    const unavailable = (interrupt = false) =>
        options.enabled === false ||
        node.dataset.backDismissed === 'true' ||
        (!interrupt && animation !== undefined && settlingCommit);
    ...
    function down(event: PointerEvent) {
        if (unavailable(true) || !event.isPrimary || event.button !== 0) return;
    ...
    return {
        update(next: BackOptions) {
            const changed =
                next.onback !== options.onback ||
                next.beforeback !== options.beforeback ||
                next.enabled !== options.enabled;
            options = next;
            if (changed || next.enabled === false) {
                cancel();
                wheel.cancel();
            }
        },
