...
{#snippet tileMark(id: StudioSection)}
    <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        {:else if id === 'memory'}
            <path d="M12 4a5 5 0 0 0-5 5v1a4 4 0 0 0 1 7v3h8v-3a4 4 0 0 0 1-7V9a5 5 0 0 0-5-5z" />
        {:else if id === 'content'}
...
<section class="orchestration-studio" aria-labelledby="orchestration-studio-title">
    ...
    {#if section === 'prompt' || section === 'memory'}
                ...
                <section class="studio-card" aria-labelledby="room-prompt-sources-title">
                    <div class="section-heading">
                        <div>
                            ...
                            <p>
                                사용자 표시 이름, 작가 메모, 그룹 문맥과 안전 템플릿 슬롯을 이 방의
                                CAS 설정으로 저장합니다.
                            </p>
                        </div>
                        <div class="row-actions">
                    ...
                    {#if orchestrationState.workspace.room_config.supported_fields.template_slots}
                        <fieldset class="structured-editor">
                            ...
                            <div class="section-heading">
                                <p>
                                    이름은 고유해야 하며 <code>block_content</code>는 예약되어
                                    있습니다.
                                </p>
                                <button
                                    type="button"
                ...
                <section class="studio-card" aria-labelledby="variables-title">
                    <div class="section-heading">
                        <div>
                            ...
                            <p>
                                값의 타입과 범위는 Core가 검증하며 템플릿에서는 값 삽입만
                                허용합니다.
                            </p>
                        </div>
                    </div>
                                                ...
                                                <div class="editor-grid">
                                                    ...
                                                    <label>
                                                        ...
                                                        <input
                                                            type="number"
                                                            min="1"
                                                            value={profile.value.rate_limit
                                                                .requests}
                                                            oninput={(event) =>
                                                                controller.stageTaskProfile(
                                                                    profile.value.id,
                            ...
                            {#each orchestrationState.workspace.memory_records as record (record.id)}
                                <li>
                                    ...
                                    <div class="row-actions">
                                        ...
                                        >
                                            {pendingMemoryDeleteId === record.id
                                                ? '삭제 확인'
                                                : '삭제'}
                                        </button>
                                ...
                                <ol class="compact-list">
                                    {#each orchestrationState.transform_preview.reports.slice(0, MAX_INLINE_ITEMS) as report, index (`${String(index)}:${report.trace.rule_id}`)}
                                        <li>
                                            ...
                                            {#if report.diff !== null}
                                                <small>
                                                    diff 앞 {report.diff.unchanged_prefix_chars}자 ·
                                                    뒤
                                                    {report.diff.unchanged_suffix_chars}자{report
                                                        .diff.truncated
                                                        ? ' · fragment 축약'
                                                        : ''}
                                                </small>
                            ...
                            <article class="package-review">
                                ...
                                <div class="row-actions">
                                    <button
                                        type="button"
                                        disabled={contentPackageState.phase !== 'ready' ||
                                            contentPackageState.selected_component_ids.length ===
                                                0 ||
                ...
                <section class="studio-card" aria-labelledby="plan-preview-title">
                    ...
                    {#if client !== undefined}
                        <GenerationAttemptApprovals
                            ...
                            sourceBranchId={orchestrationState.workspace.room_config.branch_id ||
                                null}
...
<style>
    ...
    @container view (max-width: 640px) {
        ...
        /*
         * The studio is the whole screen here, so its own frame would only
         * wrap the page in a second border and spend a fifth of the width on
         * concentric padding. The cards inside keep theirs.
         */
        .orchestration-studio {
            padding: 0;
            border: 0;
        ...
        /*
         * A card header is a title beside its controls only while there is
         * room for both. In a phone-width container the title keeps the full
         * measure and the controls drop beneath it, rather than each being
         * squeezed into a column too narrow to read.
         */
        /*
         * Stacked, the copy must take the whole column: left to `flex-start`
         * it sizes to its own longest line instead and drags the grid wider
         * than the window. Only the controls below it keep a natural width.
         */
    }
</style>
