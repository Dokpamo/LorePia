...
>
    ...
    {:else if section === null}
        ...
        />
    {:else if section === 'ai'}
        <div class="provider-scroll settings-detail-scroll"><AiSettingsHome onSelect={openSettingsShortcut} /></div>
    ...
    {:else if section === 'prompt' || section === 'memory' || section === 'plugins' || section === 'storage'}
        {#if services}<div class="provider-scroll settings-detail-scroll">{#key section}<PlaySettings {services} {section} bind:detailPage />{/key}</div>{/if}
