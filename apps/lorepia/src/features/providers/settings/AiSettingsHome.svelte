<script lang="ts">
    ...
    const tools = ['discovery', 'templates', 'catalog'] as const;
...
<div class="settings-workspace">
            ...
            <button class="settings-purpose-row" onclick={() => onSelect(section)}>
                ...
                <span
                    ><strong>{$tr(`settings.section.${section}.title`)}</strong><small
                        >{$tr(`settingsLive.${section}Hint`)}</small
                    ></span
                >
    ...
    <div class="settings-purpose-card">
        {#each tools as section (section)}
            <button class="settings-purpose-row" onclick={() => onSelect(section)}
                ><span><strong>{$tr(`settings.section.${section}.title`)}</strong></span
