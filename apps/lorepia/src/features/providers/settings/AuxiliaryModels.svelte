...
<div class="settings-models">
    ...
    <div class="settings-inline-actions">
        <button
            ...
            disabled={documentsState.busy}>{$tr('settingsLive.addSummaryModel')}</button
        >
        <button
            onclick={() => {
            ...
            disabled={documentsState.busy}>{$tr('settingsLive.addEmbeddingModel')}</button
        >
    </div>
    {#each documentsState.tasks.filter((item) => item.value.kind === 'memory_summary' || item.value.kind === 'memory_embedding') as item (item.value.id)}
        <button
            ...
            ><span
                ...
                ><small
                    >{workspace.routes.find((route) => route.id === item.value.route_id)
                        ?.display_name ??
                        workspace.routes.find((route) => route.id === item.value.route_id)
                            ?.model_id ??
                        item.value.route_id}</small
                ></span
    ...
    {#if draft}
        ...
        >
            ...
            />
            {#if draft.kind === 'memory_embedding'}<label
                    >{$tr('settingsUi.dimensions')}<input
                        type="number"
                        bind:value={draft.embedding_dimensions}
                        min="1"
                        max="32768"
                        required
                        disabled={documentsState.busy}
                ...
                >{/if}
            <label
                >{$tr('settingsLive.timeout')}<input
                    type="number"
                    bind:value={draft.timeout_ms}
                    min="1"
                    max="600000"
                    required
                    disabled={documentsState.busy}
            ...
            >
            <label
                >{$tr('settingsLive.requestCount')}<input
                    type="number"
                    bind:value={draft.rate_limit.requests}
                    min="1"
                    max="10000"
                    required
                    disabled={documentsState.busy}
            ...
            >
            <label
                >{$tr('settingsLive.requestSeconds')}<input
                    type="number"
                    bind:value={draft.rate_limit.per_seconds}
                    min="1"
                    max="3600"
                    required
                    disabled={documentsState.busy}
            ...
            >
            <label
                >{$tr('settingsLive.concurrency')}<input
                    type="number"
                    bind:value={draft.concurrency_limit}
                    min="1"
                    max="32"
                    required
                    disabled={documentsState.busy}
            ...
            <div class="settings-inline-actions">
                <button class="primary" type="submit" disabled={documentsState.busy}
                    >{$tr('settingsLive.saveModel')}</button
                ><button type="button" disabled={documentsState.busy} onclick={() => (draft = null)}
