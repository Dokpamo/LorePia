<script lang="ts">
    ...
    } from '../../features/chat/portable-regex';
...
<div class="ui-import-backdrop">
    <dialog
        class="ui-import-review"
        aria-modal="true"
    ...
    >
        <header class="ui-page-header ui-navigation-header">
            <IconButton
                label={$tr('import.dialog.close')}
                onclick={() => void controller.discardImport()}><ArrowLeft /></IconButton
            >
        </header>
        <div class="ui-import-content">
            <h1 id="import-review-title" class="ui-detail-title">{$tr('import.title')}</h1>

            {#if appState.import_flow.phase === 'loading'}
                <div class="ui-settings-row">
                    <div class="ui-settings-row" role="status">
            ...
            {:else if appState.import_flow.phase === 'error'}
                <div class="ui-settings-row">
                    <div class="ui-settings-row" role="alert">
                        ...
                        {#if appState.import_flow.resource_override_available}
                            ...
                            <button
                                ...
                                ><span class="ui-press-visual">{$tr('import.resource.retry')}</span
                                ></button
                            >
                        {/if}
                        <button
            ...
            {:else if appState.import_flow.inspection}
                ...
                <div class="ui-settings-row">
                    <div class="ui-settings-row">
                        <span class="ui-settings-row" aria-hidden="true"
                            >{inspection.display_name.slice(0, 1)}</span
                        >
                        <div>
                            <div class="ui-settings-row">
                                <h3 class="ui-settings-heading">
                                    {importedText(inspection.display_name)}
                                </h3>
                                <span class="ui-settings-row">{kindLabel(inspection.kind)}</span>
                            </div>
                            <p class="ui-settings-row">
                    ...
                    {#if appState.import_flow.resource_override_active}
                        <section class="ui-settings-row" aria-labelledby="large-import-title">
                            <h3 class="ui-settings-heading" id="large-import-title">
                                {$tr('import.resource.title')}
                            </h3>
                    ...
                    {#if isImportedContent(inspection.kind)}
                        <section
                            ...
                            aria-labelledby="compatibility-import-title"
                        >
                            <h3 class="ui-settings-heading" id="compatibility-import-title">
                                {$tr('import.compatibility.title')}
                            </h3>
                    ...
                    {#if inspection.dynamic_content.runtime_script_count > 0 || inspection.dynamic_content.regex_rule_count > 0 || inspection.dynamic_content.custom_markup_present}
                        <section class="ui-settings-row" aria-labelledby="dynamic-content-title">
                            <h3 class="ui-settings-heading" id="dynamic-content-title">
                                {$tr('import.dynamic.title')}
                            </h3>
                            <ul>
                    ...
                    {#if inspection.blocked_reasons.length > 0}
                        <section class="ui-settings-row" aria-labelledby="blocked-title">
                            <h3 class="ui-settings-heading" id="blocked-title">
                                {$tr('import.blocked')}
                            </h3>
                            <ul>
                    ...
                    {#if inspection.warnings.length > 0}
                        <section class="ui-settings-row" aria-labelledby="warning-title">
                            <h3 class="ui-settings-heading" id="warning-title">
                                {$tr('import.warnings')}
                            </h3>
                            <ul>
                ...
                <footer class="ui-settings-row">
                    <button
                        class="ui-submit ui-pressable"
                        type="button"
                        onclick={() => void controller.discardImport()}
                        ><span class="ui-press-visual">{$tr('import.cancel')}</span></button
                    >
                    <button
                        class="ui-submit ui-pressable"
                        type="button"
...
<style>
    ...
    .ui-import-review[open] {
        display: flex;
        flex-direction: column;
    }
    .ui-import-review::backdrop {
    ...
    .ui-import-content {
        ...
        min-height: 0;
        overflow-y: auto;
        padding: var(--ui-space-8) var(--ui-inset) var(--ui-space-6);
    }
    .ui-import-content > .ui-settings-row {
        display: block;
    }
    .ui-import-content section {
        align-items: stretch;
    }
    .ui-import-content p {
        overflow-wrap: anywhere;
    }
    .ui-import-content footer {
        display: flex;
        flex-direction: row;
        gap: var(--ui-space-4);
    }
