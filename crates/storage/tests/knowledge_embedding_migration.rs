...
const LAST_INVERTED_SCHEMA_VERSION: u32 = 43;
...
fn downgrade_and_seed_populated_v31(fixture: &FixtureIds, vector_blob: &[u8], vector_sha256: &str) {
    ...
    transaction
        .execute_batch(include_str!(
            "../src/database/pending_checkpoints/drop_test_schema.sql"
        ))
        .expect("remove schema 43 checkpoints");
    transaction
...
fn restore_schema40_package_capability_requests(transaction: &Transaction<'_>) {
    transaction
        .execute_batch(include_str!(
...
fn remove_schema40_objects(transaction: &Transaction<'_>) {
    ...
    assert_eq!(
        ...
        "schema-31 inverse must track every schema-40 additive object"
    );
    for object_type in ["VIEW", "TRIGGER", "INDEX", "TABLE"] {
            ...
            .filter(|(candidate_type, _)| *candidate_type == object_type)
        {
            transaction
                ...
                .unwrap_or_else(|error| panic!("remove schema-40 {object_type} {name}: {error}"));
        }
    }
}
