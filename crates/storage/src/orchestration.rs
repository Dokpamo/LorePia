...
impl Storage {
    ...
    pub fn soft_delete_knowledge_book(
        ...
        let current = self.get_knowledge_book(id)?;
        soft_delete_content_object(
            self,
            DocumentTable::KnowledgeBooks,
            id.as_str(),
    ...
    pub fn soft_delete_memory_profile(
        ...
        let current = self.get_memory_profile(id)?;
        soft_delete_content_object(
            self,
            DocumentTable::MemoryProfiles,
            id.as_str(),
    ...
    pub fn get_memory_record(
        ...
        get_memory_record(self, id, false)
    }
    ...
    pub fn soft_delete_memory_record(
        &self,
        id: &MemoryRecordId,
        expected_revision: u64,
    ) -> CoreResult<StoredRevision<MemoryRecord>> {
        soft_delete_memory_record(self, id, expected_revision)
    }
    ...
    let sql = format!(
        "SELECT revision.document_json, state.state_version,
         ...
         JOIN memory_record_revisions AS revision
          ...
          AND revision.id = state.active_revision_id
         WHERE record.id = ?1{deleted_clause}"
    );
    let raw = storage
        .connection()?
        .query_row(&sql, [id.as_str()], raw_memory_record)
        .optional()
        .map_err(storage_db_error)?
...
fn soft_delete_memory_record(
    storage: &Storage,
    id: &MemoryRecordId,
    expected_revision: u64,
) -> CoreResult<StoredRevision<MemoryRecord>> {
    let current = get_memory_record(storage, id, false)?;
    if current.revision != expected_revision {
        return Err(revision_conflict(
            "memory record",
            id.as_str(),
    ...
    append_memory_event(
        ...
        id.as_str(),
        "deleted",
        current.revision_id.as_deref(),
        current.revision_id.as_deref(),
        serde_json::json!({"state_version": next_revision}),
        now,
    )?;
    transaction.commit().map_err(storage_db_error)?;
    Ok(StoredRevision {
...
fn resolve_memory_profile_projection_authority(
    ...
    let summary_schema_revision =
        ensure_memory_summary_schema(transaction, &profile.summary_schema)?;
...
fn validate_memory_profile(profile: &MemoryProfile) -> CoreResult<()> {
    ...
    if profile.name.trim().is_empty()
        ...
        || profile.turns_per_summary == 0
        || weights
            .iter()
            .any(|weight| !weight.is_finite() || !(0.0..=1.0).contains(weight))
        || weights.iter().sum::<f32>() <= 0.0
    {
        return Err(CoreError::invalid("memory profile policy is invalid"));
    }
    Ok(())
}
...
fn ensure_memory_summary_schema(
    ...
    let provenance = Provenance {
        source_kind: SourceKind::ApplicationBuiltIn,
        source_id: Some(id.as_str().to_owned()),
        source_hash: None,
        author: Some("LorePia".to_owned()),
        license: None,
        imported_at: None,
...
fn write_knowledge_book_projection(
    ...
    if book.name.trim().is_empty()
        || book.schema_version == 0
        || (!book.recursive && book.max_recursion_depth != 0)
    {
        return Err(CoreError::invalid(
            "knowledge book name, version, or recursion policy is invalid",
        ));
    }
    let state_version = expected_revision.map_or(1, |value| value.saturating_add(1));
    let now = Utc::now().to_rfc3339();
