...
impl Storage {
    ...
    pub fn promote_package_assets(
        ...
        let mut intents = Vec::with_capacity(staged_assets.len());
        for asset in staged_assets {
            if !identities.insert(asset.sha256.clone()) {
                return Err(CoreError::invalid(
                    "package asset digest is duplicated in the promotion set",
                ));
            }
            let _ = validate_owned_staged_file(&self.root, &asset.staged_path)?;
            let relative = content_relative_path(&asset.sha256)?;
            intents.push(PackageCasPromotionIntent {
                import_id: import_id.to_owned(),
                namespace: "asset",
...
mod tests {
    ...
    fn assert_attemptless_generation_has_no_terminal_lifecycle(
        storage: &Storage,
        generation_id: &str,
    ) {
