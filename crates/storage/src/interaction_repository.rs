...
fn read_recent_reopen_effect_history(
    connection: &Connection,
    conversation_id: &ConversationId,
    branch_id: &ConversationBranchId,
    limit: u32,
...
fn read_older_reopen_effect_history(
    connection: &Connection,
    conversation_id: &ConversationId,
    branch_id: &ConversationBranchId,
    before: InteractionEffectHistoryCursor,
...
fn read_pending_choice_effect_history(
    connection: &Connection,
    conversation_id: &ConversationId,
    branch_id: &ConversationBranchId,
    limit: u32,
...
fn require_pending_choice_effect(
...
fn validate_effect_poll_limit(limit: u32) -> CoreResult<()> {
...
fn validate_proposal_list_limit(limit: u32) -> CoreResult<()> {
    if limit == 0 || limit > 1_024 {
        return Err(CoreError::invalid(
...
fn validate_stored_effect_identity(
    effect_id: &str,
    event_id: &str,
    sequence: i64,
) -> CoreResult<()> {
...
fn decode_choice_effect_lifecycle(
...
fn validate_effect_delivery_token(
    event_id: &str,
    sequence: u64,
    delivery_attempts: u64,
) -> CoreResult<()> {
...
fn interaction_effect_id(event_id: &str, sequence: i64) -> String {
    let mut hasher = Sha256::new();
