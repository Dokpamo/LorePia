...
type TextChunk<'a> = (&'a [u8], Cow<'a, [u8]>);
    ...
    loop {
        ...
        if let Some((keyword, encoded)) = decoded {
            ...
            if keyword == V2_KEYWORD && legacy.is_none() {
                legacy = Some(encoded.into_owned());
            }
        }
    }
...
fn read_text_chunk(payload: &[u8]) -> Option<TextChunk<'_>> {
    ...
    Some((&payload[..separator], Cow::Borrowed(text)))
}

/// Splits a `zTXt` payload and inflates it under a hard output ceiling.
fn read_compressed_text_chunk(payload: &[u8]) -> CoreResult<Option<TextChunk<'_>>> {
