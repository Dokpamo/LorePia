...
const EXTERNAL_MODULE_MAGIC: [u8; 2] = [111, 0];
const EXTERNAL_MODULE_HEADER_BYTES: usize = 6;
...
pub(super) struct ImportedAssetRecord {
...
pub(super) struct ImportedModuleSource {
    pub(super) metadata: DecodedRuntimeMetadata,
    pub(super) assets: Vec<ImportedAssetRecord>,
}

pub(super) fn is_external_module(path: &Path) -> CoreResult<bool> {
    let mut file = File::open(path).map_err(storage_error)?;
    let mut header = [0_u8; 2];
    let read = file.read(&mut header).map_err(storage_error)?;
    Ok(read == header.len() && header == EXTERNAL_MODULE_MAGIC)
}

pub(super) fn read_external_module(
    path: &Path,
    limits: ImportLimits,
    source_sha256: &str,
) -> CoreResult<ImportedModuleSource> {
    let mut reader = BufReader::new(File::open(path).map_err(storage_error)?);
    let mut header = [0_u8; EXTERNAL_MODULE_HEADER_BYTES];
    reader
        .read_exact(&mut header)
        .map_err(|_| unsupported("imported module header is truncated"))?;
    if header[..2] != EXTERNAL_MODULE_MAGIC {
        return Err(unsupported("imported module header is invalid"));
    }
    let metadata_len = usize::try_from(u32::from_le_bytes(
        header[2..6].try_into().expect("four-byte length"),
    ))
    .map_err(|_| unsupported("imported module metadata does not fit this device"))?;
    if metadata_len == 0 || metadata_len > MAX_MODULE_METADATA_BYTES {
        return Err(unsupported(
            "imported module metadata exceeds the 4 MiB limit",
        ));
    }
    let mut encoded = vec![0_u8; metadata_len];
    reader
        .read_exact(&mut encoded)
        .map_err(|_| unsupported("imported module metadata is truncated"))?;
    ...
    Ok(ImportedModuleSource { metadata, assets })
}
...
fn scan_asset_records(
    ...
    limits: ImportLimits,
) -> CoreResult<Vec<ImportedAssetRecord>> {
    if metadata.len() > limits.max_entries {
        return Err(unsafe_archive(
            "imported module exceeds the entry-count limit",
        ));
    }
    ...
    for asset in metadata {
        ...
        reader
            ...
            .map_err(|_| unsafe_archive("imported asset record is truncated"))?;
        if record_type[0] != 1 {
            return Err(unsafe_archive("imported asset record type is invalid"));
        }
        let mut length = [0_u8; 4];
        reader
            .read_exact(&mut length)
            .map_err(|_| unsafe_archive("imported asset length is truncated"))?;
        ...
        while remaining > 0 {
            ...
            reader
                ...
                .map_err(|_| unsafe_archive("imported asset payload is truncated"))?;
        ...
        records.push(ImportedAssetRecord {
            offset,
            size_bytes,
    ...
    reader
        .read_exact(&mut end)
        .map_err(|_| unsafe_archive("imported module is missing its final record"))?;
    if end[0] != 0 {
        return Err(unsafe_archive(
            "imported module has an invalid final record",
        ));
    }
    let mut trailing = [0_u8; 1];
    if reader.read(&mut trailing).map_err(storage_error)? != 0 {
        return Err(unsafe_archive("imported module has trailing bytes"));
    }
    Ok(records)
}
...
fn validate_asset_signature(
    ...
    let (extension, media_type) = if signature.starts_with(b"\x89PNG\r\n\x1a\n") {
        ...
        return Err(unsupported(format!(
            "imported asset has an unsupported media signature: {} ({})",
...
pub(super) fn copy_decoded_asset<W: std::io::Write>(
    ...
    while remaining > 0 {
        ...
        source
            .read_exact(&mut buffer[..wanted])
            .map_err(|_| unsafe_archive("imported asset changed while preparing import"))?;
        decode_substitution_in_place(&mut buffer[..wanted])?;
...
pub(super) fn decode_external_preset(path: &Path) -> CoreResult<Value> {
    ...
    if encoded.is_empty() || encoded.len() as u64 > MAX_PRESET_CONTAINER_BYTES {
        return Err(unsupported(
            "imported preset exceeds the 16 MiB container limit",
        ));
    }
    ...
    GzDecoder::new(encoded.as_slice())
        ...
        .map_err(|error| unsupported(format!("imported preset compression is invalid: {error}")))?;
    if decompressed.len() as u64 > MAX_PRESET_CONTAINER_BYTES {
        return Err(unsupported(
            "imported preset container expands beyond 16 MiB",
        ));
    }
    let outer = read_messagepack(&decompressed, "imported preset container")?;
    if map_text(&outer, "type") != Some("preset") || map_u64(&outer, "presetVersion") != Some(2) {
        return Err(unsupported(
            "imported preset type or version is unsupported",
        ));
    }
    let encrypted = map_value(&outer, "preset")
        .or_else(|| map_value(&outer, "pres"))
        .and_then(MessagePackValue::as_slice)
        .ok_or_else(|| unsupported("imported preset has no encrypted payload"))?;
    if encrypted.len() > MAX_PRESET_DOCUMENT_BYTES {
        return Err(unsupported("imported preset payload exceeds 16 MiB"));
    }
    let key = Sha256::digest(b"risupreset");
    let key = UnboundKey::new(&AES_256_GCM, key.as_slice())
        .map_err(|_| unsupported("imported preset key could not be initialized"))?;
    ...
    let plaintext = cipher
        ...
        .map_err(|_| unsupported("imported preset authentication failed"))?;
    if plaintext.len() > MAX_PRESET_DOCUMENT_BYTES {
        return Err(unsupported("imported preset document exceeds 16 MiB"));
    }
    let value = read_messagepack(plaintext, "imported preset document")?;
...
fn messagepack_to_json(
    ...
    if depth > MAX_MESSAGEPACK_DEPTH {
        return Err(unsupported("imported preset nesting exceeds the limit"));
    }
    *nodes = nodes.saturating_add(1);
    if *nodes > MAX_MESSAGEPACK_NODES {
        return Err(unsupported(
            "imported preset structure exceeds the node limit",
        ));
    }
    match value {
        ...
        MessagePackValue::Map(values) => {
            ...
            for (key, value) in values {
                let key = key
                    .as_str()
                    .ok_or_else(|| unsupported("imported preset map key is not text"))?;
                if map.contains_key(key) {
                    return Err(unsupported("imported preset contains duplicate map keys"));
                }
                map.insert(
                    key.to_owned(),
...
fn storage_error(error: std::io::Error) -> CoreError {
    CoreError::new(
        ...
        format!("cannot read or write imported compatibility data: {error}"),
        true,
    )
}
...
mod tests {
    ...
    fn streams_module_assets_and_corrects_declared_media_type() {
        let metadata = serde_json::to_vec(&json!({
            "type": "risuModule",
            "module": {
                "name": "Synthetic external module",
        ...
        let module = read_external_module(source.path(), ImportLimits::default(), &"a".repeat(64))
            .expect("read module");
        assert_eq!(module.metadata.name, "Synthetic external module");
    ...
    fn authenticates_and_decodes_a_version_two_preset() {
        ...
        let decoded = decode_external_preset(source.path()).expect("decode preset");
        assert_eq!(
